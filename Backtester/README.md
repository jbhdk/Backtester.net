...
- **Coverage floor & priming** — the cache-aware fetcher records the earliest range start ever asked of the provider (a per-symbol+interval sidecar) and refuses a run that starts before it with a `DataCoverageException`, rather than silently serving a short slice. `HistoricalDataFetcher.PrimeAsync` warms a wide range up front so in-sample and out-of-sample sub-ranges run entirely from the cache.
- **Performance stats** — win rate, profit factor, expectancy, max drawdown, CAGR, Sharpe, and more are computed from completed round trips and surfaced as `PerformanceStats`: the report package hands you the block as `new ReportModelBuilder().Build(result).Stats`, and an Optimizer scores each `Trial.Stats` with them. The `Portfolio` itself exposes the live account figures — `Cash`, `RealizedPnL`, `MarkedEquity`, `RealizedEquity`, `BuyingPower`.
...
// The account figures are on the portfolio; for the full stats block (win rate, profit factor,
// max drawdown, Sharpe, …) add backtester.net.report and build its model from this same result:
// PerformanceStats-shaped numbers arrive as new ReportModelBuilder().Build(result).Stats.
decimal finalEquity = result.Portfolio.MarkedEquity;
```
...
| `Backtester.Data` | `IHistoricalDataProvider`, `IHistoricalDataFetcher`, `IWarmupResolvingFetcher`, `HistoricalDataFetcher`, `CsvHistoricalDataFetcher`, `CsvBarLoader`, `DataCoverageException`, `InsufficientWarmupBarsException` |
