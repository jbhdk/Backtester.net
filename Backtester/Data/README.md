...
- Live network providers ship as their own opt-in packages, so the core stays network-free (see `docs/adr/0009-network-providers-separate-packages.md`): `backtester.net.data.yahoo` (Yahoo Finance v8), `backtester.net.data.alpaca` (Alpaca), and `backtester.net.data.oanda` (Oanda v20 forex). The only provider in the core package is the offline `CsvHistoricalDataFetcher`. If a provider cannot serve the requested interval it throws `NotSupportedException`.
